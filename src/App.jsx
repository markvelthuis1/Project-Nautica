          ...
          <div key={si} style={{ display: "flex", alignItems: "center" }}>
            ...
            {si < 5 && (
              <div className="connector" style={{
                background: isGreen && steps[si + 1] === 2 ? "#F5A623"
                          : isBlue  && steps[si + 1] >= 1  ? "#4AB8D8"
                          : "#D8E4F0",
              }} />
            )}
          </div>
...
export default function StepTracker() {
  ...
  const [expanded, setExpanded]     = useState({ "arch-2": true });
  const [hoveredKey, setHoveredKey] = useState(null);
  const [modal, setModal]           = useState(null);
  const [newName, setNewName]       = useState("");

  const toggleFeatureStep = (epic, featureId, si) =>
    setData(prev => ({ ...prev, [epic]: prev[epic].map(f =>
      f.id === featureId ? { ...f, steps: f.steps.map((s, i) => i === si ? (s + 1) % 3 : s) } : f
    )}));

  const toggleSubstep = (epic, featureId, substepId, si) =>
    setData(prev => ({ ...prev, [epic]: prev[epic].map(f =>
      f.id === featureId ? { ...f, substeps: f.substeps.map(sub =>
        sub.id === substepId ? { ...sub, steps: sub.steps.map((s, i) => i === si ? (s + 1) % 3 : s) } : sub
      )} : f
    )}));

  const openModal = (epic, mode, featureId) => { setModal({ epic, mode, featureId }); setNewName(""); };

  const confirmAdd = () => {
    if (!newName.trim()) return;
    if (modal.mode === "feature") {
      const id = `f-${Date.now()}`;
      setData(prev => ({ ...prev, [modal.epic]: [...prev[modal.epic], { id, name: newName.trim(), steps: mkSteps(), substeps: [] }] }));
    } else {
      const id = `s-${Date.now()}`;
      setData(prev => ({ ...prev, [modal.epic]: prev[modal.epic].map(f =>
        f.id === modal.featureId ? { ...f, substeps: [...f.substeps, { id, name: newName.trim(), steps: mkSteps() }] } : f
      )}));
    ...
    <div style={{ minHeight: "100vh", background: "#F5F7FA", fontFamily: "'DM Mono','Courier New',monospace", padding: "40px 32px", color: "#1B2F5E" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .global-bar-track { background: #E8EEF5; border-radius: 999px; height: 8px; overflow: hidden; flex: 1; }
